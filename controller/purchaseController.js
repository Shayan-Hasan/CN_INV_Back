...
const getPurchaseOrderVendorByID = (req, res) => {
  const { po_id } = req.body;
  const sql =
    "select  v.vendor_id, v.name,p.amount_paid,a.account_id, p.vendor_invoice_no,p.status_id, pop.recv_by, p.ship_method, p.tracking_no, p.po_note, pop.recv_by from " +
...
const GetSpecialOrderProdDetail = (req, res) => {
  ...
  console.log(req.body);
  const sql =
    "SELECT i.unit_instock, i.opening_balance, p.code,p.discount, p.name AS productname,pi.image, p.details, p.product_id, u.name AS unit, p.unit_price, " +
    ...
    "inner JOIN inv.product_images pi ON p.product_id = pi.product_id left join inv.sale_order_products sp on p.product_id = sp.product_id " +
