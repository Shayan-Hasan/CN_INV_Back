...
const getAllSalesByID = (req, res) => {
  ...
  const sql =
    "select distinct SO.so_id,coalesce(concat(SO.est_sale, SO.so_id), '-') as 'Q-S #', DATE_FORMAT(SO.est_date, '%m/%d/%Y %l:%i:%s %p') as 'Quote Date', SO.project_name as 'Proj', SO.customer_po_no as 'Customer PO', s.name as 'Status'," +
...
const getSaleOrderCustomerByID = (req, res) => {
  const { so_id } = req.body;
  const sql =
    "select c.customer_id,a.account_id, c.name, s.customer_po_no, s.project_name, s.store_id,s.status_id,s.amount_paid, s.ship_method, s.tracking_no,i.shipment, i.tax,s.so_note, " +
...
const GetShipmentDetailByid = (req, res) => {
  ...
  console.log(so_id);
  const sql =
    "select sp.shipment_id, sp.quantity, sp.product_id,pp.details,pp.code, coalesce(sp.quantity_shipped,0) as qty_ship, coalesce(date(sp.date_shipped),'') as date_shipped, sp.note, " +
